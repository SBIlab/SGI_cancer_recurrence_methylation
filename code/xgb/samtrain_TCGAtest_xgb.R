...
#xgb_model_function
xgb = function(i, x_R, y_R, x_NR, y_NR, sam_x_R, sam_y_R, sam_x_NR, sam_y_NR){
  set.seed(2*i)
...
for(n in 1:length(toget_colcombi)){
  ...
  if(n<=length(TIL_colcombi)){
    ...
    #modeling
    for (i in 1:100){
      toget_output = xgb(i, toget_x_R, toget_y_R$Disease.Free.Status_int, toget_x_NR, toget_y_NR$Disease.Free.Status_int,
      ...
      if(n <= length(TIL_colcombi)){
        ...
        TIL_output = xgb(i, TIL_x_R, TIL_y_R$Disease.Free.Status_int, TIL_x_NR, TIL_y_NR$Disease.Free.Status_int,
        ...
        PBMC_output = xgb(i, PBMC_x_R, PBMC_y_R$Disease.Free.Status_int, PBMC_x_NR, PBMC_y_NR$Disease.Free.Status_int,
...
dir.create("../../result")
write.table(output_toget_perf, "../../result/Toget_xgb_samtrain_TCGAtest.txt", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(output_TIL_perf, "../../result/TIL_xgb_samtrain_TCGAtest.txt", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(output_PBMC_perf, "../../result/PBMC_xgb_samtrain_TCGAtest.txt", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
